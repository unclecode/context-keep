# MAINTAINING — keep the context-keep fragment set (and this skill) current

The "sub-skill": the protocol for updating `docs/context/` fragments after code changes. Read it when
running `/keep-context sync`. (This system was scaffolded by the global `codemap` skill.)
...
.claude/skills/keep-context/MAP.md  (source file → fragment, for routing + drift)
```
...
2. **Detect drift:** `bash .claude/skills/keep-context/scripts/drift.sh [range]` → (a) changed
...
6. **Regenerate:** `python3 .claude/skills/keep-context/scripts/build-map.py` — warns on any
